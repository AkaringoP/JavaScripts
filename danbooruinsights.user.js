      ...
      try {
        ...
        if (!isYearCompleteCache && metric === "uploads" && year < ( new Date()).getFullYear()) {
          try {
            ...
            const localRecords = await table.where("id").between(
...
async getMilestones(userInfo, isNsfwEnabled = false, customStep = "auto") {
      ...
      const matches = await this.db.posts.where("no").anyOf(targets).filter((p) => p["uploader_id"] === uploaderId).toArray();
      const missingIds = [];
      matches.forEach((p) => {
...
async cleanupStaleData(currentUserId) {
      ...
      const THRESHOLD = 14 * 24 * 60 * 60 * 1e3;
...
async renderDashboard() {
    ...
    `;
      ...
      `;
          ...
          btn.onclick = async () => {
            ...
            await this.dataManager.syncAllPosts(this.context.targetUser, null);
            try {
              ...
              if (topCopyrights.length > 0) {
                const bar2 = syncDiv.querySelector("#analytics-main-bar");
                const percent2 = syncDiv.querySelector("#analytics-main-percent");
                const countText2 = syncDiv.querySelector("#analytics-main-count");
                await this.dataManager.fetchBubbleData(this.context.targetUser, topCopyrights, (c, t, _msg) => {
                  const p = t > 0 ? Math.round(c / t * 100) : 0;
                  bar2.style.width = `${p}%`;
                  percent2.textContent = `${p}%`;
                  countText2.textContent = `Fetching Analytics: ${c} / ${t}`;
                });
              }
            } catch (err) {
    ...
    `;
        ...
        if (pieData.breasts) {
          ...
          const total2 = data.reduce((acc, c) => acc + c.count, 0);
          pieData.breasts = data.map((d) => ({
            ...d,
            frequency: total2 > 0 ? d.count / total2 : 0,
            value: total2 > 0 ? d.count / total2 : 0,
            label: d.name,
        ...
        const onPieDataUpdate = (e) => {
          ...
          const key = keyMap[contentTypeStr];
          if (key && pieData[key]) {
        ...
        const renderPieContent = () => {
          ...
          if (!chartWrapper) {
           ...
           `;
            ...
            container.appendChild(legendDiv2);
          }
          const width = 180;
          const height = 180;
...
"appealed": "#bf3989"
        ...
        const renderTopPostContent = () => {
          ...
          const refreshBtn2 = topPostContainer.querySelector("#analytics-random-refresh");
          if (refreshBtn2) {
    ...
    `;
        ...
        topPostContainer.addEventListener("click", (e) => {
          if (e.target.classList.contains("top-post-tab")) {
            currentTab = e.target.getAttribute("data-mode") ?? "sfw";
            updateTabs();
            renderTopPostContent();
          }
        });
          ...
          monthly.forEach((mo, idx) => {
            ...
            if (stars.length > 0) {
              ...
              stars.forEach((m, si) => {
                ...
                let fill = "#ffd700";
                let stroke = "#b8860b";
                let style2 = "filter: drop-shadow(0px 1px 1px rgba(0,0,0,0.3));";
                let animClass = "";
                if (m.index === 1) {
                  fill = "#00e676";
                  stroke = "#00a050";
                ...
                svg += `
                   <a href="/posts/${m.post.id}" target="_blank" style="cursor: pointer; pointer-events: all;" onclick="event.stopPropagation()">
                      <text class="${animClass}" x="${x}" y="${y}" text-anchor="middle" dominant-baseline="central" font-size="12" fill="${fill}" stroke="${stroke}" stroke-width="0.5" style="${style2}">
                         ★
      ...
      `;
        ...
        if (scatterData.length > 0) {
          ...
          header2.textContent = "📊 Post Performance";
...
e: { label: "E", color: "#f44336" }
          ...
          const renderScatter = () => {
            ...
            let minDate2 = Infinity;
            let maxDate2 = -Infinity;
            let maxVal = 0;
            if (selectedYear) {
              minDate2 = new Date(selectedYear, 0, 1).getTime();
              maxDate2 = new Date(selectedYear, 11, 31, 23, 59, 59).getTime();
              ...
              for (const d of scatterData) {
                if (d.d < minDate2) minDate2 = d.d;
                if (d.d > maxDate2) maxDate2 = d.d;
              }
              if (minDate2 === Infinity) {
                minDate2 = Date.now();
                maxDate2 = minDate2 + 864e5;
              } else {
                const startY = new Date(minDate2).getFullYear();
                minDate2 = new Date(startY, 0, 1).getTime();
              }
            }
            const timeRange = maxDate2 - minDate2 || 1;
            for (const d of scatterData) {
              if (d.d >= minDate2 && d.d <= maxDate2) {
            ...
            Object.assign(currentScale, { minDate: minDate2, maxDate: maxDate2, maxVal, timeRange, padL, padT, drawW, drawH, mode: currentScatterMode });
            const visiblePoints = scatterData.filter((d) => {
              if (d.d < minDate2 || d.d > maxDate2) return false;
            ...
            if (selectedYear) {
              ...
              const startYear = new Date(minDate2).getFullYear();
              const endYear = new Date(maxDate2).getFullYear();
              for (let y = startYear; y <= endYear; y++) {
                const d = new Date(y, 0, 1).getTime();
                const x = padL + (d - minDate2) / timeRange * drawW;
                if (x >= padL - 5 && x <= w - padR + 5) {
                  const nextD = new Date(y + 1, 0, 1).getTime();
                  const xNext = padL + (nextD - minDate2) / timeRange * drawW;
            ...
            visiblePoints.forEach((pt) => {
              if (pt.d < minDate2 || pt.d > maxDate2) return;
              const val = currentScatterMode === "tags" ? pt.t || 0 : pt.s;
              const x = padL + (pt.d - minDate2) / timeRange * drawW;
              const y = padT + drawH - val / maxVal * drawH;
              let color = "#ccc";
            ...
            const addOverlayLine = (dateObjOrStr, color, title, isDashed, thickness = "2px") => {
              ...
              if (d < minDate2 || d > maxDate2) return;
              const x = padL + (d - minDate2) / timeRange * drawW;
         ...
         `;
            ...
            const countLabel2 = popover.querySelector("#pop-count-label");
            if (loadMoreBtn) {
              loadMoreBtn.onclick = () => {
                ...
                countLabel2.textContent = `${Math.min(visibleLimit, totalCount)} / ${totalCount} items`;
...
async run() {
      ...
      try {
        ...
        if (rawCache) {
          ...
          const isStale = age >= 24 * 60 * 60 * 1e3;
...
async _fetchAndRender() {
      ...
      try {
        ...
        if (cachedData) {
          ...
          const isTimeExpired = age >= 24 * 60 * 60 * 1e3;
          let postCountDiff = 0;
          try {
        ...
        if (initialPosts && totalCount <= MAX_OPTIMIZED_POSTS && initialPosts.length >= totalCount) {
          const historyData2 = this.calculateHistoryFromPosts(initialPosts);
          ...
          const [statusCounts2, latestPost2, trendingPost2, trendingPostNSFW2, newPostCount2, commentaryCounts2] = await Promise.all([
            ...
            this.backfillUploaderNames(initialPosts)
]);
          ...
          if (meta.category === 1) {
            ...
            const filteredCopyright = (await Promise.all(
              copyrightCandidates.map(
                async ([tag, count]) => await isTopLevelTag(this.rateLimiter, tag) ? [tag, count] : null
              )
            )).filter((e) => e !== null);
...
latestPost,
        ...
        if (runDelta && baseData) {
          ...
          historyPromise = this.fetchHistoryDelta(tagName, deltaStart, startDate).then((delta) => this.mergeHistory(baseData.historyData, delta));
          milestonesPromise = historyPromise.then((fullHistory) => {
            return this.fetchMilestonesDelta(tagName, totalCount, baseData.precalculatedMilestones, fullHistory).then((delta) => this.mergeMilestones(baseData.precalculatedMilestones, delta));
          });
...
async _fetchNewPostCountV1(tagName) {
...
dateStr,
      ...
      const promises = tasks.map((task) => {
        const params = new URLSearchParams({
          tags: `${tagName} date:${task.queryDate}`
});
        const url = `/counts/posts.json?${params.toString()}`;
...
renderPieChart(type, tagData) {
      ...
      const getColor = (key) => {
        ...
        if (type === "commentary") {
          ...
          if (key === "has:comments -commentary -commentary_request") return "#6c757d";
        }
      ...
      g.selectAll("path").on("mouseover", function(event, d) {
        ...
        tooltip.html(`<strong>${d.data.name}</strong>: ${d.data.count.toLocaleString()} (${percent}%)`).style("left", event.pageX + 10 + "px").style("top", event.pageY - 20 + "px");
      }).on("mousemove", function(event) {
