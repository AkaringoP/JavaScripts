...
setGrassIndex(themeKey, index) {
      ...
      if (this.settings.grassIndex !== void 0) {
        delete this.settings.grassIndex;
      }
      this.save(patch);
    }
...
async fetchRemoteCount(tags) {
  ...
  const buildCardHtml = (post) => {
    ...
    const tagLine = (icon, label, value) => value ? `<div style="font-size:11px;color:#444;"><strong>${icon} ${label}:</strong> ${escapeHtml(value)}</div>` : "";
    const tagsBlock = artist || copyright || character ? `<div style="margin-top:6px;border-top:1px solid #eee;padding-top:6px;display:flex;flex-direction:column;gap:2px;">
...
case "notes":
      ...
      window.cal.paint(buildPaintConfig()).then(() => {
        ...
        setTimeout(() => {
          ...
          if (isTouchDevice2) {
            d3__namespace.selectAll("#cal-heatmap-scroll rect").on("touchstart", function(event) {
...
async fetchPromotionDate(userName) {
      ...
      const promoEvent = history2.find((h) => targetRoles.some((r) => h.role.includes(r)));
      if (promoEvent) {
...
dataManager.getUserStats(user),
    ...
    const renderPieContent = () => {
      ...
      if (isTouchDevice2) {
        const handleSliceTouch = (event) => {
          ...
          d3__namespace.select(target).transition().duration(200).attr("d", (td) => arcHover(td) ?? "").style("opacity", "1").style("filter", "drop-shadow(0px 0px 8px rgba(255,255,255,0.4))");
          let html = "";
        ...
        svg.selectAll("path.danbooru-grass-pie-path").on("touchstart", function(event) {
        ...
        document.addEventListener("touchstart", (e) => {
          ...
          if (tooltipEl && !tooltipEl.contains(e.target) && !svgEl?.contains(e.target)) {
            tooltip.style("opacity", 0);
            svg.selectAll("path.danbooru-grass-pie-path").transition().duration(200).attr("d", (td) => arc(td) ?? "").style("opacity", "0.9").style("filter", "none");
            lastTouchedPieDatum = null;
          }
  ...
  async function renderMilestonesWidget(container, db, context, initialNsfwEnabled) {
    ...
    const renderMilestones = async () => {
      ...
      const milestones = await dm.getMilestones(context.targetUser, isNsfwEnabled, currentMilestoneStep);
  ...
  function renderScatterPlot(container, scatterData, context, levelChanges, options = {}) {
    ...
    const updateDownvoteButtonStyles = () => {
      downvoteButtons.forEach((btn) => {
        ...
        btn.title = isDisabled ? backfillFailed ? "Downvote data unavailable (fetch failed)" : "Backfilling downvote data..." : `Show only posts with more than ${t} downvotes`;
      });
    };
    ...
    document.addEventListener("click", (e) => {
      ...
      if (e.target === y10Hit || y10Tooltip.contains(e.target)) return;
      closeY10Tooltip();
    });
    ...
    canvas.style.cursor = isTouchDevice2 ? "default" : "crosshair";
    const popover = document.createElement("div");
  ...
  function renderTagCloudWidget(container, options) {
    ...
    if (isTouchDevice2) {
      document.addEventListener("click", (e) => {
        ...
        if (svgEl && !svgEl.contains(e.target) && !cloudTooltip.contains(e.target)) {
          resetCloudHighlight();
        }
      });
    ...
    `;
        ...
        if (scatterData.length > 0) {
          renderScatterPlot(dashboardDiv, scatterData, this.context, levelChanges, {
            userStats,
            needsBackfill,
            runBackfill: needsBackfill ? (onProgress) => dataManager.backfillPostMetadata(this.context.targetUser, onProgress) : void 0,
...
async fetchRatingCounts(tagName, startDate = null) {
    ...
    async fetchHistoryBackwards(tagName, forwardStartDate, targetTotal, currentForwardTotal) {
      ...
      console.log(`[TagAnalyticsApp] Reverse Scan Completed. Total: ${totalSum}/${targetTotal}, Months Checked: ${history2.length} (hits)`);
      let runningSum = 0;
...
renderPieChart(type, tagData) {
      ...
      g.selectAll("path").on("mouseover", function(event, d) {
        ...
        tooltip.html(`<strong>${escapeHtml$1(d.data.name)}</strong>: ${d.data.count.toLocaleString()} (${percent}%)`).style("left", event.pageX + 10 + "px").style("top", event.pageY - 20 + "px");
      }).on("mousemove", function(event) {
      ...
      milestonePosts.forEach((item) => {
        ...
        card.innerHTML = `
            ...
            <div style="font-size: 0.75em; color: #888; margin-top: 4px; line-height: 1.2; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
                <a href="/users/${p.uploader_id}" target="_blank" class="${getLevelClass(p.uploader_level)}" style="text-decoration: none;">${escapeHtml$1(uploaderName)}</a>
            </div>
...
renderAreaChart(data, selector, title) {
      ...
      svg.append("g").attr("transform", `translate(0,${height - margin.top - margin.bottom})`).call(d3__namespace.axisBottom(x).ticks(tickCount).tickFormat((d) => {
...
buildDashboardHeader(tagData, titleColor, categoryLabel) {
      ...
      <div class="di-tag-header" style="border-bottom: 1px solid #eee; padding-bottom: 15px; margin-bottom: 20px; display: flex; justify-content: space-between; align-items: flex-end;">
          <div>
              <h2 style="margin: 0 0 5px 0; color: ${titleColor};">${escapeHtml$1(tagData.name.replace(/_/g, " "))}</h2>
