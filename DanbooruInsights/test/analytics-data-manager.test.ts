...
describe('AnalyticsDataManager.getBestThumbnailUrl', () => {
  it('빈 post(null)이면 빈 문자열 반환', () => {
    expect(AnalyticsDataManager.getBestThumbnailUrl(null)).toBe('');
  });
  ...
  it('variants에 720x720 webp가 있으면 그 URL 반환', () => {
    ...
    expect(AnalyticsDataManager.getBestThumbnailUrl(post)).toBe('http://example.com/720.webp');
  });
  ...
  it('720x720 webp 없으면 360x360 webp 반환', () => {
    ...
    expect(AnalyticsDataManager.getBestThumbnailUrl(post)).toBe('http://example.com/360.webp');
  });
  ...
  it('webp 없으면 preferred type(720x720) 중 첫 번째 반환', () => {
    ...
    expect(AnalyticsDataManager.getBestThumbnailUrl(post)).toBe('http://example.com/720.jpg');
  });
  ...
  it('preferred type 없으면 첫 번째 variant URL 반환', () => {
    ...
    expect(AnalyticsDataManager.getBestThumbnailUrl(post)).toBe('http://example.com/original.png');
  });
  ...
  it('variants가 빈 배열이면 preview_file_url fallback', () => {
    ...
    expect(AnalyticsDataManager.getBestThumbnailUrl(post)).toBe('http://example.com/preview.jpg');
  });
  ...
  it('variants 없으면 file_url fallback', () => {
    ...
    expect(AnalyticsDataManager.getBestThumbnailUrl(post)).toBe('http://example.com/file.jpg');
  });

  it('모든 fallback 없으면 빈 문자열', () => {
    expect(AnalyticsDataManager.getBestThumbnailUrl({})).toBe('');
  });
});
