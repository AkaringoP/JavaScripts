...
export class SidebarInjector {
    ...
    toggleGroupMenu(btn, tagName) {
        ...
        if (existingMenu) {
            ...
            // @ts-ignore
            if (existingMenu._triggerBtn === btn)
                return;
        }
        const isDark = detectDarkTheme();
        ...
        addBtn.onmouseenter = () => { if (addLabel.tagName === 'DIV')
            addLabel.style.opacity = '1'; };
        addBtn.onmouseleave = () => { if (addLabel.tagName === 'DIV')
            addLabel.style.opacity = '0'; };
        addBtn.onclick = (e) => {
            e.stopPropagation();
