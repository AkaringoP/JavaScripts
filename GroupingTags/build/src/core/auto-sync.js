...
export class AutoSyncManager {
     ...
     */
    static init() {
        console.log("🔄 AutoSyncManager: Initializing...");
    ...
    static notifyChange(postId) {
        ...
        console.log(`⏳ AutoSync: Scheduled for Shard ${shardIdx} (Debounce 5s)`);
        // 3. Reset Timer
        if (this.syncTimeout)
    ...
    static checkPendingSync() {
        ...
        if (elapsed >= DEBOUNCE_TIME) {
            ...
            console.log("🔄 AutoSync: Found pending overdue sync. Executing now.");
            this.executeSync();
        }
        else {
            // Timer still running -> Resume wait
            const remaining = DEBOUNCE_TIME - elapsed;
            console.log(`⏳ AutoSync: Resuming pending sync in ${remaining}ms`);
    ...
    static async executeSync() {
        ...
        if (dirtyShards.length === 0)
            return;
        console.log("🔄 AutoSync: Starting Batch Sync...", dirtyShards);
        ...
        console.log("✅ AutoSync: Batch Pending Finished.");
    }
