...
export class SmartInputHandler {
  ...
  constructor(selector: string, checkEnabled: () => boolean) {
    ...
    this.input = document.querySelector(selector) as HTMLTextAreaElement;
    if (this.input) {
      this.init();
    } else {
      console.warn(
  ...
  private onKeyDown(e: KeyboardEvent) {
    ...
    if (this.isComposing) return; // Ignore keys during composition
  ...
  private handleInput(e: InputEvent | null) {
    if (!this.checkEnabled()) return;
  ...
  private onSelectionChange() {
    ...
    if (this.isDeleting) return;
    if (this.isComposing) return; // Don't jump cursor while composing
  ...
  private insertText(textToInsert: string, cursorOffset: number) {
    if (!this.input) return;
