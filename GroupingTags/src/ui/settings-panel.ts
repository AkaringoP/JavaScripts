...
import { ImportManager, mergeGroups } from '../core/import-manager';
import { getLocalDataByShard, savePostTagData } from '../db';
...
export class SettingsPanel {
     ...
     */
    private static async renderPanelContent(panel: HTMLElement, isDark: boolean) {
        ...
        importBtn.onclick = async () => {
                ...
                if (conflicts.length > 0) {
                    ...
                    ConflictModal.show(conflicts, async (res) => {
                        ...
                        for (const c of conflicts) {
                            let final = c.remote;
                            if (res === 'MERGE' && c.local) final = { ...c.local, groups: mergeGroups(c.local.groups, c.remote.groups), isImported: true };
                            else if (res === 'OVERWRITE') final.isImported = true;

                            if (res !== 'KEEP') { await savePostTagData(final); count++; }
                        }
            ...
            finally { importBtn.disabled = false; importBtn.textContent = 'Start Import'; }
        };
