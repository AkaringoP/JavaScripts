...
import { detectDarkTheme, stringToColor } from '../../utils';
...
export class ConflictModal {
     ...
     */
    static show(diffs: DiffResult[], onResolve: (resolution: 'MERGE' | 'OVERWRITE' | 'KEEP') => void) {
        ...
        diffs.slice(0, 50).forEach(d => { // Show max 50 to prevent lag
    ...
    private static createBtn(text: string, bgColor: string, onClick: () => void) {
